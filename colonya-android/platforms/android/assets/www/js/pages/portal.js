...
portal = (function () {
    ...
    var obtenerUrlAcceso = function () {
        var ent = config.headers.CODSecEnt;
        portal.accesoClienteUrl = dataHelper.getAccess().Url + "&ISUM_ID_USUARIO=" + userHelper.getCampoCifrado() + "&ID_PERSONA=" + userHelper.getCampoPersona() + "&ID_DISPOSITIVO_MOVIL=" + push.getRegID() + "&ISUM_UA=" + dataHelper.getGoogleAnalytics() + "&ISUM_APP=1" + "&entidad=" + ent;
        console.log("obtenerUrlAcceso:  "+portal.accesoClienteUrl);
    };

    var irAAvisos = function () {
        if (utils.comprobarConexion()) {
            userHelper.reloadActivePushUsers();
        } else {
   ...
   var irACampana = function (urlCampana) {
    ...
    var cbAppMovil = function (res) {
        ...
        EE_O_PortalPubAPP.setCache(JSON.parse(localStorage.getItem("respuestaPortalPubb")).EE_O_PortalPubAPP.Respuesta);
        EE_O_PortalPubAPP.setRespeti(JSON.parse(localStorage.getItem("respuestaPortalPubb")).EE_O_PortalPubAPP.codigoRetorno);
    ...
    var readApplicationData = function () {
        ...
        // console.log("jsonParam:::::::::"+jsonParam.codigoEntidad);

        ///////////////////////////////
        //////parche para udefined/////
        ...
        if(localStorage.getItem("noLlamarPortalPubb") === true || localStorage.getItem("noLlamarPortalPubb") === "true") {
            console.log("noLlamarPortalPubb");
            cbAppMovil();
            downloadData();
            comprobarNumAvisos();
            dataHelper.noLlamarPortalPubb(false);
        } else {
            console.log("SILLAMARAPORTALPUBB");
            // alert("SILLAMARAPORTALPUBB");
            ...
            $.ajax({
                ...
                success: function(data, response) {
                    console.log("data readApplicationData: "+JSON.stringify(data));
                    localStorage.setItem("respuestaPortalPubb",JSON.stringify(data));
    ...
    return {
        ...
        comprobarNumAvisos: comprobarNumAvisos
    };
}());
